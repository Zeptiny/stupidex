...
description: 'Phase 2 divergent ideation dispatch spec — fleet, frames, per-idea contract, post-merge synthesis.'
---
...
Dispatch parallel ideation sub-agents per the Model Tiers fleet. The default fleet is **5 agents covering all six frames**:
...
- `<background>` — everything else in the grounding (codebase context, additional context, RAG research results, user-supplied research): informative, not directive — it can supply an idea's basis, but it must not pull ideation toward whatever was loudest in the corpus when the user named a different focus
