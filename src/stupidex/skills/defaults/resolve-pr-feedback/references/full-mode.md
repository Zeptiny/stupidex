---
description: Full mode workflow for resolving all unresolved PR review threads -- fetch, triage, plan, parallel implement, validate, commit/push, reply/resolve, verify, summary.
---
...
Create a task list of all **new** unresolved items -- one entry per thread or comment to resolve.
...
1. **Run the project's validation command** (test suite, type check, or whatever the repo's AGENTS.md specifies). Run once, not per-agent.
...
Present the decisions and wait for the user's response. After they decide, process the remaining items: fix the code, compose the reply, post it, and resolve the thread.
