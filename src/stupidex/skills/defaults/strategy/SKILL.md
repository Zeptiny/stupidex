---
name: strategy
description: 'Create or maintain STRATEGY.md -- the product\'s target problem, approach, users, key metrics, and tracks of work. Use when starting a new product, updating direction, or when prompts like "write our strategy" or "what are we working on" come up.'
---
...
1. **Anchor, not plan.** Strategy is what the product is and why. Features belong in `brainstorm`; schedules belong in the issue tracker. Do not let either creep into the doc.
...
Read `STRATEGY.md` using the `read` tool.
...
If no specific target, ask the user which section to revisit. Options:
...
After writing, note in one line where the file lives and that `ideate`, `brainstorm`, and `plan` will pick it up as grounding on their next run.

If no downstream skill has run yet on this repo, suggest `ideate` or `brainstorm` skills as a next step.
