...
Callers invoke headless mode by including `mode:headless` in the skill arguments.
...
**If no document is specified (interactive mode):** Ask which document to review, or find the most recent in `docs/brainstorms/` or `docs/plans/` using the platform's file-search/glob tool.

**If no document is specified (headless mode):** Output "Review failed: headless mode requires a document path. Re-invoke with the document path as an argument." without dispatching agents.
