---
name: bulk-preview
description: 'Defines the compact plan preview shown before bulk actions in interactive mode.'
---
...
After the preview body is rendered, ask the user using the platform's blocking question tool. Only when the harness genuinely lacks a blocking tool, fall back to presenting numbered options and waiting for the user's next reply. Never silently skip the question.
