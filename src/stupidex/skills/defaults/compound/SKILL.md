---
name: compound
description: 'Document a recently solved problem to compound your team's knowledge. Use when the user says "document this", "compound this learning", "what did we learn", or after fixing a bug.'
---
...
Invoke the `compound` skill with optional context:
- No arguments: Document the most recent fix
- Brief context hint: Provide additional context
- `mode:headless`: Non-interactive run for automations

## CONCEPTS.md bootstrap requests

If invoked specifically to create or bootstrap `CONCEPTS.md` from scratch rather than to document a solved problem, do not run the normal phases — `compound` populates `CONCEPTS.md` only as a side effect of documenting a real learning (it seeds the *learning's area*, not the whole repo; see Phase 2.4). Repo-wide concept-map creation is the `compound-refresh` skill's job. Redirect a standalone bootstrap request to `compound-refresh` (which asks whether to build the concept map or run a refresh cycle), then exit.

## Mode Detection

Check the skill arguments for a `mode:headless` token. Tokens starting with `mode:` are flags, not context — strip `mode:headless` from arguments before treating the remainder as the brief context hint.
...
These files are the durable contract for the workflow. Read them on-demand at the step that needs them — do not bulk-load at skill start. The skill tool automatically discovers and lists them.
...
**In interactive mode**, ask the user directly in conversation to choose between the following options. Present numbered options and wait for the user's reply.
...
If the user says yes, invoke the `sessions` skill in Phase 1 (see step 4). If no, skip it. Do not ask this in lightweight mode or headless mode.
...
- **A project instruction file** (e.g., AGENTS.md) — a small edit when the Discoverability Check finds a gap.
...
1. Look for a block labeled "user's auto-memory" already present in your system prompt context — MEMORY.md's entries are inlined there
...
**Dispatch order:**
- Launch `Context Analyzer`, `Solution Extractor`, and `Related Docs Finder` in parallel using `delegate_to_subagent`
- **Then** invoke the `sessions` skill (see step 4 below) — only if the user opted in to session history. The skill call is synchronous from this orchestrator's main-context turn, but the already-dispatched background subagents continue running in parallel underneath, so the wall-clock benefit is preserved.
...
#### 3. **Related Docs Finder**
   ...
   **Search strategy (grep-first filtering for efficiency):**
   ...
   3. Use the `grep` tool to pre-filter candidate files BEFORE reading any content. Run multiple searches in parallel, case-insensitive, targeting frontmatter fields. These are template patterns — substitute actual keywords:
      - `title:.*<keyword>`
   ...
   **GitHub issue search:**
   ...
   Run `gh issue list --search "<keywords>" --state all --limit 5` via `execute_command`. If `gh` is not installed, skip GitHub issue search and note it was skipped in the output.

</parallel_tasks>

#### 4. **Session History** (synchronous skill call, after launching the parallel block — only if the user opted in)
   ...
   - Invoke the `sessions` skill. Pass the dispatch payload below as the skill argument string. `sessions` runs in main context — it owns discovery, branch/keyword filtering, scan-window selection, and the deep-dive cap. The compound orchestrator only needs to pass the topic and time window and read back the findings text.
   ...
   **Dispatch payload — keep tight.** A long, keyword-rich payload licenses sessions to keep widening. Use this shape:
   ...
   Do not append additional context blocks, exclusion lists, or topic-keyword bullets — verbose payloads give the sessions skill license to keep widening the search and rapidly compound wall time. If keyword search is needed, sessions owns that decision internally based on the topic.
...
6. Create directory if needed: `mkdir -p docs/solutions/[category]/` via `execute_command`
7. Write the file: either the updated existing doc or the new `docs/solutions/[category]/[filename].md`
8. **Run the frontmatter validator** via `execute_command`: `python3 scripts/validate-frontmatter.py <output-path>` to catch silent-corruption parser-safety issues that the prose rules miss: malformed `---` delimiter lines, unquoted ` #` in scalar values (silent comment truncation), and unquoted `: ` in scalar values (silent mapping confusion). Exit 0 means the doc is parser-safe; exit 1 means the script's stderr names the offending field(s) and what to fix — quote the value(s), re-write the doc, and re-run until exit 0. Do not declare success while validation fails. The script does not enforce schema rules and does not flag YAML reserved-indicator characters (those produce loud parser errors downstream rather than silent corruption — out of scope). Uses Python 3 stdlib only (no PyYAML or other deps).
...
1. Identify which root-level instruction files exist (e.g., AGENTS.md). Read the file(s) and determine which holds the substantive content — one file may just be a shim that includes the other. The substantive file is the assessment and edit target; ignore shims. If no instruction file exists, skip this check entirely.
...
4. If not:
   ...
   b. Draft the smallest addition that communicates the three things. Match the file's existing style and density. The addition should describe the knowledge store itself, not the skill — an agent without the skill should still find value in it.
      ...
      ```
   c. In full interactive mode, explain to the user why this matters — agents working in this repo (including fresh sessions, other tools, or collaborators without the skill) won't know to check `docs/solutions/` unless the instruction file surfaces it. Show the proposed change and where it would go, then ask the user directly in conversation for consent before making the edit. In lightweight mode, output a one-liner note and move on. In headless mode, apply the edit directly without prompting and surface it in the terminal report under "Instruction-file edit"
...
Based on problem type, optionally invoke specialized agents to review the documentation via `delegate_to_subagent`:

- **performance_issue** → `performance-oracle`
- **security_issue** → `security-sentinel`
- **database_issue** → `data-integrity-guardian`
- Any code-heavy issue → always run `code-simplicity-reviewer` for minimal, clear examples.
...
1. **Extract from conversation**: Identify the problem and solution from conversation history. Also scan the "user's auto-memory" block injected into your system prompt, if present — use any relevant notes as supplementary context alongside conversation history. Tag any memory-sourced content incorporated into the final doc with "(auto memory)"
...
4. **Vocabulary capture (update-only)**: if `CONCEPTS.md` exists at repo root, read `references/concepts-vocabulary.md`, then scan the new doc and the conversation for qualifying terms and add/refine entries silently (same criteria as Phase 2.4). Do **not** bootstrap or seed in lightweight mode — if `CONCEPTS.md` does not exist, defer creation to a Full run, which owns seeding. Record the outcome in the output (e.g., "Vocabulary: 1 entry refined" or "scanned, no qualifying terms"). If you refined `CONCEPTS.md` and a quick read of instruction files shows it isn't surfaced there, add the discoverability tip to the output below — lightweight **tips**, it does not edit instruction files (a Full run owns that edit).
...
**After displaying the interactive success output above, ask the user directly in conversation** for their choice from the "What's next?" options. Do not continue the workflow or end the turn without the user's selection. (Interactive mode only — headless skips this per the headless block above.)
...
<manual_override> Use the compound skill with context to document immediately without waiting for auto-detection. </manual_override> </auto_invoke>
...
Based on problem type, these agents can enhance documentation via `delegate_to_subagent`:
