...
description: 'Template for spawning validator sub-agents in Stage 5b. Independent re-verification of each surviving finding before externalization.'
---
...
2. **Is the issue introduced by THIS diff?** Use git blame or diff inspection. If the cited line predates this PR's commits and the diff does not interact with it, the finding is pre-existing — not validated for externalization regardless of whether it is a real issue.

3. **Is the issue not handled elsewhere?** Look for guards in callers, middleware in the request chain, framework defaults, type system constraints, or parallel handlers that already address the concern.
...
| `{finding_reviewer}` | Stage 5 merged finding | Original persona name (informational) |
