...
Usage:
    ...
    python -m examples.ask_question_cli '{"questions": [{"title": "Favorite color?", "choices": ["Red", "Blue", "Green"]}]}'

    # Pass via stdin (pipe)
    echo '{"questions": [{"title": "Pick one", "choices": ["A", "B"]}]}' | python -m examples.ask_question_cli
...
def main() -> None:
    ...
    if raw_input:
        ...
        if not specs:
            print("Error: No questions provided in JSON.", file=sys.stderr)
            sys.exit(1)
    else:
    ...
    xml = _format_answers_xml(result or [])
